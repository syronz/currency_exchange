...
class loan_payment extends database{
	private static $TABLE = 'loan_payment';
	...
	public static function calculate_row(){
		try{
			...
			echo 'Error: [loan_payment.class.php/function calculate_row]'.$e->getMessage().'<br>';
			die();
		}
	}
	...
	private static function calculate_balance($id_account){
		try{
			...
			echo 'Error: [loan_payment.class.php/function calculate_balance]'.$e->getMessage().'<br>';
			die();
		}
	}
	public static function lists($sorting,$startIndex,$pageSize){
		try{
			...
			echo 'Error: [loan_payment.class.php/function lists]'.$e->getMessage().'<br>';
			die();
		}
	}
	...
	public static function last_row_data(){
		try{
			...
			echo 'Error: [loan_payment.class.php/function last_row_data]'.$e->getMessage().'<br>';
			die();
		}
	}
	...
	public static function create($data){
		try{
			...
			if(@$data['loss']){
				$data_fund['detail'] .= ' - '.dic_return('loss').' / '.$data_fund['a_dollar'];
			...
			echo 'Error: [loan_payment.class.php/function create]'.$e->getMessage().'<br>';
			die();
		}
	}
	...
	public static function update($data){
		try{
			...
			$fund_info = fund::get_fund_info2('loan_payment',$data['id']);
			$accounts_info = accounts::get_accounts_info($data['id_account']);
			// dsh($fund_info);
			$data_fund = array(
							'type' => 'loan_payment',
			...
			echo 'Error: [loan_payment.class.php/function update]'.$e->getMessage().'<br>';
			die();
		}
	}
	...
	public static function delete($id){
		try{
			...
			$fund_info = fund::get_fund_info2('loan_payment',$id);
			// dsh($fund_info);
			...
			echo 'Error: [loan_payment.class.php/function delete]'.$e->getMessage().'<br>';
			die();
		}
	}
	...
	public static function get_loan_payment_info($id){
		try{
			$sql = "SELECT p.id,p.date_time,p.a_dollar,p.a_dinar,p.a_tman,p.detail,a.owner_name FROM ".self::$TABLE." p left join accounts a on a.id = p.id_account WHERE p.id = :id";
			$stmt = self::$PDO->prepare($sql);
			$stmt->bindParam(':id',$id,PDO::PARAM_INT);
			$stmt->execute();
			...
			echo 'Error: [loan_payment.class.php/function get_loan_payment_info]'.$e->getMessage().'<br>';
			die();
		}
	}
}
...
// dsh(loan_payment::lists('id ASC',0,10));
...
// 		'loss' => '1'
// );
// dsh(loan_payment::update($data));
// dsh(loan_payment::create($data));
// dsh(loan_payment::delete(55));
?>
